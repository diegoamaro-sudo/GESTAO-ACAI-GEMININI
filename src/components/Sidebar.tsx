...
const Sidebar = () => {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r border-white/10 bg-card md:flex">
      <div className="flex h-96 items-center justify-center border-b border-white/10 px-2">
        <Link to="/" className="flex items-center gap-2 font-semibold">
          {config?.logo_url ? (
            <img src={config.logo_url} alt="Logo da Loja" className="h-80 w-auto object-contain" />
          ) : (
