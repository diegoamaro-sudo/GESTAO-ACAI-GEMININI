...
export const ComposicaoDialog = ({ open, onOpenChange, onSuccess, composicao }: ComposicaoDialogProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (custoItens.length > 0) {
      const itensToInsert = custoItens.map(({ id, ...rest }) => ({ ...rest, produto_id: produtoId }));
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <Button type="button" onClick={() => { setSelectedCustoItem(null); setIsCustoItemDialogOpen(true); }}>
                ...
                <TableBody>
                  {custoItens.map(item => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right">{item.custo_unitario.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell>
